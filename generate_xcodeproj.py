...
APP_DIR = os.path.join(ROOT, "MenuBarCalendar")
TEST_DIR = os.path.join(ROOT, "MenuBarCalendarTests")
OUT = os.path.join(ROOT, "MenuBarCalendar.xcodeproj", "project.pbxproj")
...
lines.append("\t\t\tremoteInfo = MenuBarCalendar;")
lines.append("\t\t};")
lines.append("/* End PBXContainerItemProxy section */")
lines.append("")
lines.append("/* Begin PBXFileReference section */")
lines.append(
    f"\t\t{ids['app_product']} /* MenuBarCalendar.app */ = {{isa = PBXFileReference; explicitFileType = wrapper.application; includeInIndex = 0; path = MenuBarCalendar.app; sourceTree = BUILT_PRODUCTS_DIR; }};"
)
lines.append(
    f"\t\t{ids['test_product']} /* MenuBarCalendarTests.xctest */ = {{isa = PBXFileReference; explicitFileType = wrapper.cfbundle; includeInIndex = 0; path = MenuBarCalendarTests.xctest; sourceTree = BUILT_PRODUCTS_DIR; }};"
)
lines.append(
...
for path in app_sources + test_sources:
    base = os.path.basename(path)
    rel_path = path
    if path.startswith("MenuBarCalendar/"):
        rel_path = path[len("MenuBarCalendar/"):]
    elif path.startswith("MenuBarCalendarTests/"):
        rel_path = path[len("MenuBarCalendarTests/"):]
    lines.append(
...
for path in app_sources:
    rel = os.path.relpath(path, "MenuBarCalendar")
...
make_group(ids["app_group"], "MenuBarCalendar", "MenuBarCalendar", app_children)

test_children = [f"\t\t\t\t{file_ref[p]} /* {os.path.basename(p)} */," for p in sorted(test_sources)]
make_group(ids["test_group"], "MenuBarCalendarTests", "MenuBarCalendarTests", test_children)

products_children = [
    f"\t\t\t\t{ids['app_product']} /* MenuBarCalendar.app */,",
    f"\t\t\t\t{ids['test_product']} /* MenuBarCalendarTests.xctest */,",
]
make_group(ids["products_group"], "Products", "", products_children)

main_children = [
    f"\t\t\t\t{ids['app_group']} /* MenuBarCalendar */,",
    f"\t\t\t\t{ids['test_group']} /* MenuBarCalendarTests */,",
    f"\t\t\t\t{ids['products_group']} /* Products */,",
]
make_group(ids["main_group"], "", "", main_children)

# Write xcscheme
scheme_dir = os.path.join(ROOT, "MenuBarCalendar.xcodeproj", "xcshareddata", "xcschemes")
os.makedirs(scheme_dir, exist_ok=True)
scheme_path = os.path.join(scheme_dir, "MenuBarCalendar.xcscheme")
  ...
  <BuildAction parallelizeBuildables="YES" buildImplicitDependencies="YES">
    <BuildActionEntries>
      <BuildActionEntry buildForTesting="YES" buildForRunning="YES" buildForProfiling="YES" buildForArchiving="YES" buildForAnalyzing="YES">
        <BuildableReference BuildableIdentifier="primary" BlueprintIdentifier="{ids['app_target']}" BuildableName="MenuBarCalendar.app" BlueprintName="MenuBarCalendar" ReferencedContainer="container:MenuBarCalendar.xcodeproj"/>
      </BuildActionEntry>
      <BuildActionEntry buildForTesting="YES" buildForRunning="NO" buildForProfiling="NO" buildForArchiving="NO" buildForAnalyzing="NO">
        <BuildableReference BuildableIdentifier="primary" BlueprintIdentifier="{ids['test_target']}" BuildableName="MenuBarCalendarTests.xctest" BlueprintName="MenuBarCalendarTests" ReferencedContainer="container:MenuBarCalendar.xcodeproj"/>
      </BuildActionEntry>
    </BuildActionEntries>
  </BuildAction>
...
lines.append(f"\t\t{ids['app_target']} /* MenuBarCalendar */ = {{")
lines.append("\t\t\tisa = PBXNativeTarget;")
lines.append(f"\t\t\tbuildConfigurationList = {ids['app_cfg']} /* Build configuration list for PBXNativeTarget \"MenuBarCalendar\" */;")
...
lines.append("\t\t\t);")
lines.append('\t\t\tname = MenuBarCalendar;')
lines.append(f"\t\t\tproductName = MenuBarCalendar;")
lines.append(f"\t\t\tproductReference = {ids['app_product']} /* MenuBarCalendar.app */;")
lines.append('\t\t\tproductType = "com.apple.product-type.application";')
lines.append("\t\t};")

lines.append(f"\t\t{ids['test_target']} /* MenuBarCalendarTests */ = {{")
lines.append("\t\t\tisa = PBXNativeTarget;")
lines.append(f"\t\t\tbuildConfigurationList = {ids['test_cfg']} /* Build configuration list for PBXNativeTarget \"MenuBarCalendarTests\" */;")
lines.append("\t\t\tbuildPhases = (")
...
lines.append("\t\t\t);")
lines.append('\t\t\tname = MenuBarCalendarTests;')
lines.append(f"\t\t\tproductName = MenuBarCalendarTests;")
lines.append(f"\t\t\tproductReference = {ids['test_product']} /* MenuBarCalendarTests.xctest */;")
...
lines.append("\t\t\t\t};")
lines.append("\t\t\t};")
lines.append(f"\t\t\tbuildConfigurationList = {ids['proj_cfg']} /* Build configuration list for PBXProject \"MenuBarCalendar\" */;")
...
lines.append(f"\t\t\ttarget = {ids['app_target']} /* MenuBarCalendar */;")
...
app_common = {
    ...
    "INFOPLIST_FILE": "MenuBarCalendar/Info.plist",
    "LD_RUNPATH_SEARCH_PATHS": "$(inherited) @executable_path/../Frameworks",
    "MARKETING_VERSION": "0.0.2",
    "PRODUCT_BUNDLE_IDENTIFIER": "com.junhey.MenuBarCalendar",
...
test_common = {
    ...
    "MACOSX_DEPLOYMENT_TARGET": "13.0",
    "MARKETING_VERSION": "0.0.2",
    "PRODUCT_BUNDLE_IDENTIFIER": "com.junhey.MenuBarCalendarTests",
    "PRODUCT_NAME": "$(TARGET_NAME)",
    "SWIFT_VERSION": "5.9",
    "TEST_HOST": "$(BUILT_PRODUCTS_DIR)/MenuBarCalendar.app/$(BUNDLE_EXECUTABLE_FOLDER_PATH)/MenuBarCalendar",
}
...
for list_id, name, cfgs in [
    ...
    (ids["app_cfg"], "MenuBarCalendar", [(ids["debug_app"], "Debug"), (ids["release_app"], "Release")]),
    (ids["test_cfg"], "MenuBarCalendarTests", [(ids["debug_test"], "Debug"), (ids["release_test"], "Release")]),
]:
